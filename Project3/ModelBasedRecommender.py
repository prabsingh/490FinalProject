...
class ModelBasedRecommender:
    ...
    def initialize(self, trainFile, testFile):
        ...
        self.trainTransposeCSR = self.trainCSR.transpose()
    ...
    def updateV(self):
        ...
        for item in range(1, numItems + 1):
            startRow = self.trainTransposeCSR.row_ptr[item - 1]
            endRow = self.trainTransposeCSR.row_ptr[item]

            cumulSum = [0] * self.k
            #for all users that have rated the item
            for i in range(startRow, endRow):
                rowU = self.trainTransposeCSR.col_ind[i] - 1
            ...
            cumulMat = np.zeros((self.k, self.k))
            for i in range(startRow, endRow):
                rowV = self.trainTransposeCSR.col_ind[i] - 1
                ui = self.U[rowU]
                ui = np.asmatrix(ui)
                uiT = np.transpose(ui)
                uiTui = np.dot(uiT, ui)
    ...
    def compareAndOutput(self, fileName):
        ...
        with open(fileName, 'w') as outputFile:
            ...
            for user in range(0,self.testCSR.rows):
                ...
                for col in range(self.testCSR.row_ptr[user], self.testCSR.row_ptr[user + 1]):
                    item = self.testCSR.col_ind[col] - 1
